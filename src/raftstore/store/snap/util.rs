...
use kvproto::raft_serverpb::{RaftSnapshotData, RegionLocalState, SnapshotMeta};
...
/// `SnapStaleNotifier` is used for checking a snapshot is stale or not. Normally the `PeerStorage`
...
pub fn stale_for_generate(key: SnapKey, snap_stale_notifier: &SnapStaleNotifier) -> bool {
...
pub fn stale_for_apply(key: SnapKey, snap_stale_notifier: &SnapStaleNotifier) -> bool {
    let compacted_term = snap_stale_notifier.compacted_term.load(Ordering::SeqCst);
    let compacted_idx = snap_stale_notifier.compacted_idx.load(Ordering::SeqCst);
...
pub mod tests {
    ...
    pub fn get_test_stale_notifier() -> SnapStaleNotifier {
        SnapStaleNotifier {
