...
use engine_traits::{
    IterOptions, Iterable, Iterator, MiscExt, Mutable, Peekable, SeekKey, WriteBatchExt, CF_GC,
    CF_WRITE,
};
...
impl WriteCompactionFilter {
    ...
    fn meet_delete_mark(&mut self, mark: &[u8]) -> Result<bool, String> {
        if self.engine.get_value_cf(CF_GC, mark)?.is_some() {
            self.handle_delete_mark(mark)?;
            Ok(true)
        } else {
            self.write_batch.put_cf(CF_GC, mark, b"")?;
    ...
    fn handle_delete_mark(&mut self, mark: &[u8]) -> Result<(), String> {
        ...
        self.write_batch.delete_cf(CF_GC, mark)?;
        self.flush_pending_writes_if_need()?;
        Ok(())
    }
